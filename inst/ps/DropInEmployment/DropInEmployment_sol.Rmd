...
ps.name = "DropInEmployment"; sol.file = paste0(ps.name,"_sol.Rmd")
# character vector of all packages you load in the problem set
libs = c("dplyr", "ggplot2","glueformula", "lfe", "patchwork", "scales", "stargazer", "zoo")
...
#< award "Import data"
Congratulations! You have earned your first award. Importing data is an important step before analyzing it. There will be more awards spread throughout this problem set. You can enter `awards()` in a code block and press the `check` button to see a list of your awards.
#>
...
#< award "Data overview"
The next step after loading the data set is to get an overview of it.  For this purpose you are now familiar with the functions `dim()` and  `head()`.
#>
...
question: What is not included in the calculation of net worth?
sc:
    - Bank loans
    - Financial assets
    - Expected profits from a business idea*
    - Housing value
success: Great, your answer is correct! Expected profits from a business idea are realized in the future. Thus, they are not included in current net value. 
failure: Your answer is not correct. Please try again.
...
#< quiz "Housing net worth shock"

parts:
  - question: By how many percentage points does housing net worth decline between 2006 and 2009 in county A? Round the result to two decimal places. 
    answer: 10
    
  - question: By how many percentage points does housing net worth decline between 2006 and 2009 in county B? Round the result to two decimal places.
    answer: 13.33
...
# Select columns fips, countyname, statename,
# hnw, households and assign it to variable ch.
ch = select(countylevel, fips, countyname, statename, hnw, households)
...
#< quiz "Decline in housing net worth"
question: Take a guess. How great was the decline in housing net worth between 2006 and 2009?
sc:
    - 4.9%
    - 6.2%
    - 9.5%*
...
#< quiz "Mean housing net worth shock"
question: Why is it problematic to compute the arithmetic mean of the housing net worth shock?
sc:
    - The calculation does not include all counties.
...
```{r "1.1 6"}
weighted.mean(ch$hnw, w = ch$households)
```
...
The `ggplot2` package offers many possibilities with which to generate graphics. It is based on the layered grammar of graphics. One creates a graph by defining components of the graph and combining them with a `+` sign. Here is an overview of functions needed in this problem set:

`ggplot(data, aes(x = ..., y = ...))` generates a ggplot object. `data` defines which data set to use for the graph and `aes(x = ..., y = ...)` determines which variables are to be plotted.

Many types of graphs can be created with the `ggplot2` package. For example, `geom_point()` makes a scatterplot, `geom_line()` draws a line plot and `geom_histogram()` produces a histogram.

With the command `geom_smooth()` one can add various types of trend lines. For example, `method = lm` adds a linear regression line and `method = loess` plots a line using local regression fitting. 

Functions `xlab()` and `ylab()` change axis labels. 

`coord_cartesian(xlim = ..., ylim = ...)` zooms into a plot. Only part of the data is then displayed.

The function `breaks_width(width, offset = 0)` from `scales` package can be used to change axis ticks. To specify x-axis ticks one can write `scale_x_continuous(breaks = breaks_width(...))` and with the command `scale_y_continuous(breaks = breaks_width(...))` set y-axis ticks. 

The style of a graph can be altered by choosing a theme. For this problem set `theme_bw()` and `theme_classic()` are used. 

For more information  and other functions from `ggplot2` package, see 
<a href="https://cran.r-project.org/web/packages/ggplot2/index.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;cran.r-project.org/web/packages/ggplot2/index.html</a> and
<a href="https://ggplot2-book.org/" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;ggplot2-book.org</a>.
#>
...
#< award "First plot"
You have created your first plot using the `ggplot2` package. Visualization is a helpful tool when interpreting data.
#>
...
#>
countyindustrylevel = readRDS("countyindustrylevel.Rds")
```
...
```{r "1.2 2"}
sample_n(countyindustrylevel, 10)
```
...
#>
industrylevel = distinct(countyindustrylevel, naics, .keep_all = TRUE)
```
...
# indcat, ntr_geog and tr_geog from data set industrylevel
# and assign it to variable il1.
il1 = select(industrylevel, naics, industry, Iemp_2007, nontradable_strict,
             indcat, ntr_geog, tr_geog)
...
Tradable and non-tradable industries shall be sorted by percentage share of total employment. By means of the `dplyr` function `mutate()`, we create the variable `Iemp_2007` which contains for each four-digit NAICS industry the share of total employment in 2007.
...
il1 = il1%>%
  mutate(
    # Calculate total employment by summing up Iemp_2007.
    totalemp = sum(Iemp_2007),
    # Calculate for each four-digit NAICS industry
    # the percentage share of total employment in 2007.
    Iempshare07 = (Iemp_2007/totalemp) * 100
  )
...
#< quiz "Non-tradable industries using retail and world trade based definition"
question: Which of the following industries is one of the top 20 non-tradable industries according to the retail and world trade classification? Multiple answers are possible. 
mc:
    - Clothing stores* 
    - Wood manufacturing
...
il1%>%
  # Generate a subset of il1 that only contains non-tradable data
  # by filtering for indcat == "non-tradable". 
  filter(indcat == "non-tradable")%>%
  # Sort industries in descending order by employment share.
  arrange(desc(Iempshare07))%>%
  # Select columns naics, industry, indcat, ntr_geog and nontradable_strict.
  select (naics, industry, indcat, ntr_geog, nontradable_strict)%>%
  # Show the first 20 rows of the data set. 
  slice_head(n = 20)
...
#< quiz "Tradable industries using retail and world trade based definition"
question: Take a guess. Which industry is the largest tradable industry using the retail and world trade based definition?

sc:
    - Semiconductor and other electronic manufacturing
...
#< quiz "Geographical Herfindahl index"
question: Consider an industry with production in two counties. 80% of the employees work in county A and 20% work in county B. What is the geographical Herfindahl index of this industry?
...
#>
il2 = select(industrylevel, naics, industry, Iherf, Ihcat, ntr_rwt, tr_rwt)
#< task
...
#< quiz "Non-tradable industries using definition based on geographical concentration"
question: Which of the following industries is classified as non-tradable under both classification schemes?
sc:
    - Florists*
...
il2%>%
  ...
  arrange(Iherf)%>%
  # Select columns naics, industry, Iherf, Ihcat and ntr_rwt. 
  select (naics, industry, Iherf, Ihcat, ntr_rwt)%>%
  # Show the first 20 rows of the data set. 
  slice_head(n = 20)
...
#< quiz "Tradable industries using definition based on geographical concentration"

question: Take a guess. Which industry has the highest Herfindahl index?
sc:
...
#< award "Dplyr commands"
You have used various `dplyr` commands in this chapter. Functions from `dplyr` package are useful to modifying data as you need it. For more information see <a href="https://cran.r-project.org/web/packages/dplyr/index.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;cran.r-project.org/web/packages/dplyr/index.html</a>.
#>
...
#< quiz "Decline in total employment"
question: Take a guess. How great was the decline in total employment between 2007 and 2009?
sc:
    - 3.4% 
    - 5.3%*
    - 8.6%
...
#< quiz "Decline in non-tradable and tradable employment"
question: What do you think? Did tradable or non-tradable employment decline more between 2007 and 2009?
sc:
    - Non-tradable employment declined more than tradable employment between 2007 and 2009. 
    - Tradable employment declined more than non-tradable employment between 2007 and 2009.*
    - It depends on which classification scheme you apply.
success: Great, your answer is correct!
failure: Your answer is not correct. Try again.
...
# Average change in non-tradable employment between 2007 and 2009:
# 1) Retail and world trade based definition:
weighted.mean(ce$ntremp_rwt, w = ce$households)
# 2) Definition based on geographical concentration:
weighted.mean(ce$ntremp_geog, w = ce$households)
	        
# Average change in tradable employment between 2007 and 2009: 
# 1) Retail and world trade based definition:
weighted.mean(ce$tremp_rwt, w = ce$households)
# 2) Definition based on geographical concentration:
weighted.mean(ce$tremp_geog, w = ce$households)
```
...
```{r "1.3 4", fig.height = 7.5, fig.width = 10.5}
#< task
...
$y$ is the dependent variable. $X = x_1, \ldots, x_k$ are the independent variables. $\varepsilon$ is called error term or disturbance term. It describes all unobserved factors, which influence the independent variable.

The OLS method estimates the parameter vector $\beta$ by minimizing the sum of squared residuals:

$$ \hat{\beta} = \text{arg min} \sum_{i=1}^n \hat{\varepsilon}_i^2 = \text{arg min} \sum_{i=1}^n (y_i - \hat{y}_i)^2 = (X'X)^{-1}X'y.$$

The OLS estimator is the best unbiased linear estimator (BLUE), if the following assumptions are fulfilled:

1. The dependent variable is a linear function of independent variables and a disturbance term $(y = \beta X + \varepsilon)$.  
2. The expected value of the disturbance term is zero $(\mathbb{E}(\varepsilon_i) = 0 \; \forall i)$.  
3. a) Homoskedasticity: Disturbances have uniform variance $(Var(\varepsilon_i) = \sigma^2 \; \forall i)$.  
b) No autocorrelation: Disturbances are uncorrelated $(Cov(\varepsilon_i, \varepsilon_j) = 0 \; \forall i \neq j)$.  
4. Observations on independent variables can be considered fixed in repeated samples.  
5. There are more observations than independent variables. In addition, there are no exact linear relationships between independent variables.  

The theory can be found in Wooldridge (2020, pp. 760-761), Kennedy (2008, pp.41-42) and Auer & Rottmann (2015, p. 460).
...
The `felm()` function from the `lfe` package can be used to run regressions. In order to regress `y` on `x1` and `x2` if all variables are stored in data set `dat`, the following command can be used: 
...
`felm()` provides a lot of features for regression analysis. One can weight regressions and cluster standard errors, for example. Some of the available options will be used in this problem set. More information on the `felm()` function can be found here: <a href="https://cran.r-project.org/web/packages/lfe/index.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;cran.r-project.org/web/packages/lfe/index.html</a>.
#>
...
#>
reg1 = felm(ntremp_rwt ~ hnw, data = countylevel)
reg2 = felm(ntremp_geog ~ hnw, data = countylevel)
```

#< award "First regression"
Nice! You have run your first regressions using the `felm()` function.
#>
...
question: If housing net worth in a county declined by 5% between 2006 and 2009, the regression model predicts that non-tradable employment would decline by approximately...
sc:
    - ... 0.63 percentage points between 2007 and 2009. 
    - ... 1.26 percentage points between 2007 and 2009.*
    - ... 2.52 percentage points between 2007 and 2009.
success: Great, your answer is correct!
failure: Your answer is not correct. Please try again.
...
```{r "2.1 4", fig.height = 5, fig.width = 10}
#< fill_in
...
# In the first plot, restaurants and the retail sector
# are defined as non-tradable. 
p1 = ggplot(subset(countylevel, households>50000 & hnw>-0.3 & abs(ntremp_rwt<0.2)),
            aes(x = hnw, y = ntremp_rwt))+
  geom_point()+
...
p2 = ggplot(subset(countylevel,
                   households>50000 & hnw>-0.3 & abs(ntremp_geog<0.2)),
            aes(x = hnw, y = ntremp_geog))+ 
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, color = "IndianRed")+
  geom_smooth(method  = "loess", se = FALSE, span = 0.8, size = 0.5,
              color = "IndianRed")+
...
#< award "Advanced plot"
Great! You combined a scatter plot with two different types of trend lines. With the `ggplot2` package you can plot different types of graphs in one plot. 
#>
...
Homoskedasticity means that all of the error terms in a regression model have the same variance. Heteroskedasticity exists when the error terms do not have the same variance (see Kennedy (2008), p. 41; Verbeek (2012), p. 15). 
#>
...
The data on the housing net worth shock refers to counties. As counties consist of different numbers of households, we use data that represents a cross-section of group averages with different group sizes. If we assume that at the household level the observations are independent and error terms are homoskedastic, following Verbeek (2012, p. 101), the variance of the disturbance term of the upper regression equation is given by:

$$ Var(\varepsilon_i|hnw_i) = \frac{\sigma^2} {n_i}, $$

where $n_i$ is the number of households in county $i$. Since disturbances do not all have the same variance, the assumption that error terms are homoskedastic is violated and heteroskedasticity is present (cf. Kennedy (2008), p.41). In this case, the OLS estimator is still unbiased and consistent, but not efficient (cf. Wooldridge (2020), pp. 262-263; Verbeek (2012), p. 94). One possibility to correct for heteroskedasticity is weighted least squares (WLS). The regression equation is multiplied by $\sqrt {n_i}$:

$$ \sqrt {n_i} \cdot ntremp\_rwt_i = \sqrt {n_i} \cdot \alpha_0 + \sqrt {n_i} \cdot \alpha_1 \cdot hnw_i + \sqrt {n_i} \cdot \varepsilon_i. $$

The weighted least squares estimator minimizes the weighted sum of squared residuals:

$$ \sum_{i=1}^n  n_i (ntremp\_rwt_i-\widehat{ntremp\_rwt_i})^2.$$

In contrast to the OLS estimator, the WLS estimator does not weight all counties equally, but gives more weight to counties with many households. More detailed information on WLS can be found in Wooldridge (2020, chapter 8-4).
#>
...
The `felm()` function from the `lfe` package allows regressions to be estimated by weighted least squares (WLS). If you want to regress `y` on `x1` and `x2` and weight the regression by variable `wgt`, the following code is used: 

```{r eval = FALSE}
felm(y ~ x1 + x2, weights = dat$wgt, data = dat)
```

For more information on the `felm()` function check <a href="https://cran.r-project.org/web/packages/lfe/index.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;cran.r-project.org/web/packages/lfe/index.html</a>.
#>
...
#>
# Run weighted regressions.
reg_wgt1 = felm(ntremp_rwt ~ hnw,
                weights = countylevel$households, data = countylevel)
reg_wgt2 = felm(ntremp_geog ~ hnw,
                weights = countylevel$households, data = countylevel)
```

#< award "Weighted regressions"
Well done! You now know how to weight regressions using `felm()` from the `lfe()` package.
#>
...
The function `felm()` function can be used to cluster standard errors. In order to use weighted least squares estimation and cluster standard errors by variable `clust`, the following command can be entered:

```{r eval = FALSE}
felm(y ~ x1 + x2|0|0|clust, weights = dat$wgt, data = dat)
```
For more detailed information see <a href="https://cran.r-project.org/web/packages/lfe/index.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;cran.r-project.org/web/packages/lfe/index.html</a>.
#>
...
#>
# Run weighted regressions with clustered standard errors.
reg_clust1 = felm(ntremp_rwt ~ hnw|0|0|statename,
                  weights = countylevel$households, data = countylevel)
reg_clust2 = felm(ntremp_geog ~ hnw|0|0|statename,
                  weights = countylevel$households, data = countylevel)
...
Nice! You clustered the standard errors using the `felm()` function.
#>
...
An explanatory variable is exogenous if it is not correlated with the error term. This means $cor(hnw_i, \varepsilon_i) = 0$. If an explanatory variable is correlated with the error term, it is endogenous, $cor(hnw_i, \varepsilon_i) \neq 0$ (cf. Stock & Watson (2019), p. 428; Wooldridge (2020), pp. 800-801). There are several reasons why endogeneity occurs. See Kennedy ((2008), pp. 139-140) for some examples. If an explanatory variable is endogenous, the OLS estimator will be biased and inconsistent. 
#>
...
# Estimate the short regression model. 
reg_short = felm(ntremp_rwt ~ hnw|0|0|statename, data = countylevel)
...
question: Consider a case in which the housing net worth shock and the share of the real estate, rental and leasing sector in employment in a county are negatively correlated and $\alpha_2$ is positive. Do we tend to underestimate or overestimate the causal relationship between the housing net worth shock and non-tradable employment?
sc:
    - We tend to underestimate the causal relationship.*
    - We tend to overestimate the causal relationship.
success: Great, your answer is correct! 
failure: Your answer is not correct. Please try again.
...
#>
# Estimate the long regression model. 
reg_long = felm(ntremp_rwt ~ hnw + empshare15|0|0|statename, data = countylevel)
...
# Compute the estimated bias.
bias = alpha2_hat * cor_hnw_empshare15 * sd_empshare15 / sd_hnw
...
Two-digit NAICS codes refer to sectors. The North American Classification System (NAICS) comprises 20 sectors. The manufacturing, retail trade and transportation and warehousing sectors are represented by several two-digit NAICS codes. We do not include the public administration sector in the analysis, as it is not included in the County Business Patterns (CBP) data set.  The list indicates to which sector each of the 23 control variables belong. 

**empshare1:** Agriculture, Forestry, Fishing and Hunting  
**empshare2:** Mining, Quarrying, and Oil and Gas Extraction  
**empshare3:** Utilities  
**empshare4:** Construction  
**empshare5:** Manufacturing  
**empshare6:**  Manufacturing  
**empshare7:** Manufacturing  
**empshare8:** 	Wholesale Trade  
**empshare9:** Retail Trade  
**empshare10:** Retail Trade  
**empshare11:** Transportation and Warehousing  
**empshare12:** Transportation and Warehousing  
**empshare13:** Information  
**empshare14:**	Finance and Insurance  
**empshare15:** Real Estate and Rental and Leasing  
**empshare16:** Professional, Scientific, and Technical Services  
**empshare17:** Management of Companies and Enterprises  
**empshare18:** Administrative and Support and Waste Management and Remediation Services  
**empshare19:** Educational Services  
**empshare20:** Health Care and Social Assistance  
**empshare21:** Arts, Entertainment, and Recreation  
**empshare22:** Accommodation and Food Services  
**empshare23:** Other Services

For more information on two-digit NAICS codes, visit <a href="https://www.census.gov/programs-surveys/economic-census/guidance/understanding-naics.html" target="_blank">&#x68;&#x74;&#x74;&#x70;&#x73;&#x3A;&#x2F;&#x2F;&#x77;&#x77;&#x77;&#x2E;census.gov/programs-surveys/economic-census/guidance/understanding-naics.html</a>.
#>
...
#< award "Control variables"
Great! You added control variables to the regression model to solve possible endogeneity problems. In the next exercise, you will learn instrumental variable estimation. This is another way to deal with endogeneity.
#>
...
#< quiz "Housing supply elasticity"

question: Consider Los Angeles County in California and Dallas County in Texas. Which county has the less elastic housing supply?
sc:
    - Los Angeles County* 
    - Dallas County
success: Great, your answer is correct!
...
question: Which relationship do you expect between housing supply elasticity and the housing net worth shock?
sc:
...
#>
# Run the regression. 
form_rel = gf(hnw ~ elasticity + {industrycontrols}|0|0|statename)
reg_relevance = felm(form_rel,
                     weights = countylevel$households, data = countylevel)
...
#>
# Run the regression. 
reg_exogeneity = felm(const_2007 ~ elasticity|0|0|statename,
                      weights = countylevel$households, data = countylevel)
...
question: The correlation between housing supply elasticity and the share of the construction sector in employment in 2007 is insignificant and close to zero. Is this good news or bad news for Mian and Sufi?
sc:
   - It is good news.* 
   - It is bad news.
success: Great, your answer is correct!
...
#< award "Valid instruments"
Excellent! You analyzed whether housing supply elasticity is a valid instrument for the housing net worth shock. While we can check the relevance condition, it is not possible to verify the exogeneity condition. We showed that housing supply elasticity is not correlated with the construction sector. It might, however, be correlated with other parts of the error term.
#>
...
We estimated the following multiple regression models in exercise 2.3, where we weighted both regressions by the number of households in a county and clustered standard errors at state level:

$$ \begin{eqnarray} 
ntremp\_rwt_i = \alpha_0 + \alpha_1 \cdot hnw_i + \alpha_2 \cdot empshare1_i + \ldots + \alpha_{24} \cdot empshare23_i + \varepsilon_i \\\
ntremp\_geog_i = \beta_0 + \beta_1 \cdot hnw_i + \beta_2 \cdot empshare1_i + \ldots + \beta_{24} \cdot empshare23_i+ u_i.
\end{eqnarray} $$

In the first stage, we regress the variable `hnw` on the instrument `elasticity` and on the control variables `empshare1`, ..., `empshare23`:

$$ hnw_i =  \gamma_0 + \gamma_1 \cdot elasticity_i + \gamma_2 \cdot empshare1_i + \ldots + \gamma_{24} \cdot empshare23_i + \nu_i. $$

Then, we compute the predicted values of this regression:

$$ \hat{hnw_i} =  \hat\gamma_0 + \hat\gamma_1 \cdot elasticity_i + \hat\gamma_2 \cdot empshare1_i + \ldots + \hat\gamma_{24} \cdot empshare23_i. $$

$\hat{hnw}$ represents the variation in the housing net worth shock, which is not correlated with the error term. In the second stage, we replace variable $hnw$ in original regressions with predicted values $\hat{hnw}$ from the first stage:

$$ \begin{eqnarray} 
ntremp\_rwt_i = \alpha_0 + \alpha_1 \cdot \hat{hnw_i} + \alpha_2 \cdot empshare1_i + \ldots + \alpha_{24} \cdot empshare23_i + \eta_i \\\
ntremp\_geog_i = \beta_0 + \beta_1 \cdot \hat{hnw_i} + \beta_2 \cdot empshare1_i + \ldots + \beta_{24} \cdot empshare23_i+ z_i.
\end{eqnarray} $$

Read more about 2SLS in Wooldridge (2020, pp. 509-511) and Stock & Watson (2019, p. 429).
#>

#< info "Instrumental variable estimation with felm()" 
The `felm()` function from the `lfe` package allows us to perform instrumental variable estimation. The following command regresses `y` on the endogenous variable `end1`, and on the exogenous variable `x2`. For the endogenous variable `end1` the instruments `z1` and `z2` are used. The third part of the formula must be placed in parentheses.

```{r eval = FALSE}
felm(y ~ x2|0|(end1 ~ z1 + z2)|clust, weights = dat$wgt, data = dat)
```
...
#>
# Perform instrumental variable estimation. 
form_iv1 = gf(ntremp_rwt ~ {industrycontrols}|0|(hnw ~ elasticity)|statename)
iv1 = felm(form_iv1, weights = countylevel$households, data = countylevel)

form_iv2 = gf(ntremp_geog ~ {industrycontrols}|0|(hnw ~ elasticity)|statename)
iv2 = felm(form_iv2, weights = countylevel$households, data = countylevel)
...
Great! You now know how instrumental variable estimation works and how to run it using the `felm()` function. If you can find valid instruments for endogenous variables, instrumental variable estimation is a good way to solve possible endogeneity problems.
#>
...
**owner**: Percentage of owner-occupied properties  
**educ_lths**: Percentage with less than high school diploma  
**educ_hs**: Percentage with only a high school diploma  
...
#< quiz "Business uncertainty hypothesis 1"
question: View the graph. Is it likely that the business uncertainty hypothesis explains the effect of the housing net worth shock and non-tradable employment? 
sc:
    - Yes, it is likely.
    - No, it is not likely.*
success: Great, your answer is correct!
...
#< quiz "Business uncertainty hypothesis 2"
question: Suppose that business uncertainty explains the positive correlation between the housing net worth shock and non-tradable employment. What relationship would you then expect to see between the change in housing net worth and the change in concerns about taxation and regulation?
sc:
...
#< quiz "Credit supply hypothesis 1"

question: If the credit supply hypothesis explained the results of exercises 2 and 3, ...
sc:
    - ... we should find a weaker correlation between the housing net worth shock and non-tradable employment among small companies. 
    - ... we should find a stronger correlation between the housing net worth shock and non-tradable employment among small companies.*
success: Great, your answer is correct!
failure: Your answer is not correct. Please try again.
...
#< quiz "Credit supply hypothesis 2"

question: If the credit supply hypothesis explained the results from exercises 2 and 3, there should be a stronger relationship between the decline in housing net worth and the decline in non-tradable employment ...
sc:
    - ... in local banking counties.* 
    - ... in national banking counties.
success: Great, your answer is correct!
failure: Your answer is not correct. Please try again.
...
countylevel.national = countylevel%>%
  filter(national == 1)
countylevel.local = countylevel%>%
  filter(local == 1)

# Perform regressions using ordinary least squares estimation.
ols8 = felm(ntremp_rwt ~ hnw|0|0|statename,
            weights = countylevel.national$households,
            data = countylevel.national)
ols9 = felm(ntremp_rwt ~ hnw|0|0|statename,
            weights = countylevel.local$households,
            data = countylevel.local)

# Perform the second stage of instrumental variable estimation.
# Use hnw.p instead of hnw as the independent variable.
iv8 = felm(ntremp_rwt ~ hnw.p|0|0|statename,
           weights = countylevel.national$households,
           data = countylevel.national)
iv9 = felm(ntremp_rwt ~ hnw.p|0|0|statename,
           weights = countylevel.local$households,
           data = countylevel.local)
...
#< award "Alternative hypotheses"
Excellent! In exercises 4 and 5, you explored whether alternative hypotheses could explain the results from previous exercises. We found no evidence that the business uncertainty hypothesis or the credit supply hypothesis are responsible for the positive correlation between the housing net worth shock and non-tradable employment.
#>
...
#< quiz "Correlation between the housing net worth shock and tradable employment"

question: Which statement is correct?
sc:
    - Counties with greater declines in housing net worth experienced greater increases in tradable employment. 
    - Counties with greater declines in housing net worth experienced sharper declines in tradable employment.
    - There is no relationship between the housing net worth shock and tradable employment at the county level.*
success: Great, your answer is correct!
failure: Your answer is not correct. Please try again.
...
```{r "6 2", fig.height = 5, fig.width = 10}
#< task
...
```{r "6 3", fig.height = 5, fig.width = 10}
#< task
...
```{r "6 5", fig.height = 5, fig.width = 10}
#< task
...
#< award "Adjustment mechanisms"
Nice! In exercise 6, you examined whether the decrease in non-tradable employment led to adjustment mechanisms in the labor market. We found no evidence that the decline in non-tradable employment was offset by an increase in tradable employment. There is little evidence that wages adjusted and that people moved from strongly impacted counties to less impacted counties.
#>
